...
`capture_rng_state`पायथन के साथ एक वाक्य वापस करता है `random.getstate`, NumPy की `np.random.get_state`प्रत्येक टुकड़ा सादे पायथन संख्याओं, tuples, और सूचियों के रूप में संग्रहीत किया जाता है (NumPy की कुंजी सरणी के माध्यम से जाता है `tolist()`), ताकि चरण 3 में लोडर इसे मनमाने वस्तुओं को अनपिल किए बिना वापस पढ़ सके। `restore_rng_state`सीपीयू tensor एक Uint8 बाइट बफर है कि PyTorch के RNG कैसे खपत करने के लिए जानता है।
...
`load_checkpoint`कॉल`torch.load(..., weights_only=True)`.`.pt`फ़ाइल एक pickle है, और unpicking एक अविश्वसनीय फ़ाइल के साथ `weights_only=False`जो भी कोड फ़ाइल नाम चलाता है. केवल वजन लोडर टेन्सर और आदिम कंटेनर स्वीकार करता है और बाकी सब कुछ खारिज करता है, यही कारण है कि चरण 1 आरएनजी राज्य को सादे सूचियों में रखता है। अखंडता जांच बढ़ जाती है`ValueError`उपयोग के बजाय `assert`, क्योंकि `python -O`पट्टियों का दावा करें. फ्लैश 2.6 या उससे अधिक का उपयोग करेंः उस रिलीज़ से पहले `weights_only=True`एक ज्ञात बायपास (CVE-2025-32434) था, इसलिए इस पाठ की गारंटी केवल 2.6 से रखती है।

### चरण 4: टुकड़े टुकड़े

`save_sharded_checkpoint`N shards के पार पैरामीटर कुंजी को गोल-रोबिन करता है, प्रत्येक shard को अपने स्वयं के परमाणु सहेज के साथ लिखता है, ऑप्टिमाइज़र और शेड्यूलर और ट्रेन राज्य के साथ एक मेटा फ़ाइल लिखता है, और shard sha256s के साथ JSON सूचकांक लिखता है। `load_sharded_checkpoint`विलय से पहले प्रत्येक टुकड़े की जांच करता है और किसी भी टुकड़े पथ को अस्वीकार करता है जो चेकपॉइंट निर्देशिका के बाहर हल हो।
...
चार पैटर्न लागू करने के लिएः

- **Load with `weights_only=True`.**साझा ड्राइव या डाउनलोड से निकाला गया चेकपॉइंट अविश्वसनीय इनपुट है। केवल वजन लोडर मशीन पर दुर्भावनापूर्ण फ़ाइल को फिर से चलाने से रोकता है।
...
-  पर PyTorch प्रलेखन`torch.save`और `torch.load`, सहित `map_location`क्रॉस-डिवाइस रिस्टोर के लिए और `weights_only`अविश्वसनीय फ़ाइलों को लोड करने के लिए।
