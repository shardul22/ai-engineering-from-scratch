...
Bir alt ip bloklisti kodu metin olarak okuyor, bu nedenle ip eşleşmesi kelimenin tam anlamıyla yazılmayan herhangi bir şeyi kaçırır.`import`Bu, bir diğer önemli konu.`__class__`ve `__globals__`Bu, gerçek tercümeciye ulaşan zincirler ve güvenli olmayan yapısal isimler yerine imletikler ile oluşturulmuştur. Yine de, bunu gerçek bir sınır değil, bir öğretim filtresi olarak düşünün. İşlemdeki herhangi bir koruma, çeviricini çalıştıran kodla paylaşıyor ve belirlenmiş bir arayan hala erişilebilir nesneleri bulabilir. Üretim sistemleri güvenilmeyen kodları ayrı bir işlem veya konteynerde (valyutaları düşmüş bir alt işlem, gVisor, Firecracker veya barındırılmış bir kod çalıştırıcısı) çalıştırır.
